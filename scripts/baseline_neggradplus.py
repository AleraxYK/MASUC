...
def baseline_neggradplus():
    """
    NegGrad+ baseline (Kurmanji et al., 2023).
    Joint loss: descent on retain set, ascent on forget set.
        L = CE(model(x_r), y_r) - beta * CE(model(x_f), y_f)
    """
    ...
    parser.add_argument("--beta",         type=float, default=0.1,  help="Ascent weight on forget set (default: 0.1)")
    parser.add_argument("--epochs",       type=int,   default=5)
    parser.add_argument("--lr",           type=float, default=0.005)
    parser.add_argument("--grad_clip",    type=float, default=1.0,  help="Max gradient norm (default: 1.0)")
    ...
    device = get_device()
    print(f"Using device: {device} | Dataset: {ds.upper()} | beta={args.beta}")
    ...
    for ep in range(1, hyperparams["epochs"] + 1):
        ...
        pbar = tqdm(retain_train_loader, desc=f"[NegGrad+] epoch {ep:02d}/{hyperparams['epochs']}", leave=False)
        for x_r, y_r in pbar:
            try:
        ...
        print(f"[NegGrad+] epoch {ep:02d}/{hyperparams['epochs']} | loss {running_loss/n:.4f} | retain_acc {metrics['retain_acc']:.4f} | forget_acc {metrics['forget_acc']:.4f} | elapsed {time.time()-t0:.1f}s")

    history["elapsed_sec"] = time.time() - t0
    history["hyperparams"] = hyperparams
    torch.save(model.state_dict(), f"results/checkpoints/{ds}_baseline_neggradplus{seed_suffix}.pth")
    with open(f"results/reports/{ds}_baseline_neggradplus{seed_suffix}_curve.json", "w") as f:
    ...
    plt.title(f"Baseline NegGrad+ ({ds.upper()}) — Retain vs Forget Accuracy")
    plt.legend(); plt.grid(True, alpha=0.3); plt.tight_layout()
    plt.savefig(f"results/plots/{ds}_baseline_neggradplus{seed_suffix}_acc.png", dpi=400, bbox_inches="tight")
    plt.close()
    print(f"Saved: results/plots/{ds}_baseline_neggradplus{seed_suffix}_acc.png")


if __name__ == "__main__":
    baseline_neggradplus()
